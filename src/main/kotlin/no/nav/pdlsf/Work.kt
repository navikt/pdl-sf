    ...
    log.info { "bootstrap work session starting" }
        ...
        getKafkaConsumerByConfig<String, String>(
            ...
            if (!cRecords.isEmpty) {
                ...
                val list = cRecords.map { consumerRecord ->
                    log.info { "Value on topic ${consumerRecord.value()}" }
                    runCatching {
                        consumerRecord.value().getQueryFromJsonString().filterPDLQuery(listOf(Filter.LEVENDE, Filter.BOSTED_TROMS_OG_FINNMARK))?.let { q ->
                            SalsforceObject(
                ...
                val toAccountCSV = list.mapNotNull { it?.accountObject }.toAccountCSV()
                val toPersonCCSV = list.mapNotNull { it?.personCObject }.toPersonCCSV()
